...
public class RingListTest {
    ...
    public void testAsList() {
        ...
        List<Integer> b = new RingList<Integer>(4);
    ...
    public void testAsQueue() {
        ...
        Queue<Integer> b = new RingList<Integer>(4);
        
        Random randA = new Random(1);
        Random randB = new Random(1);
        
        for(int trial = 0; trial < trials; trial++) {
            int ops = mRand.nextInt(500);
            
            for(int i = 0; i < ops; i++) {
                randomQueueOp(randA, a, range);
                randomQueueOp(randB, b, range);
            }
    ...
    public void testMaxCap() {
        RingList<Integer> buf = new RingList<Integer>( 4, 20 );
        ...
        try {
            buf.add( 3 );
            assertTrue( "Failed to catch allocation limit.", false );
        } catch( Exception ex ) {}
        
        assertFalse( "Failed to catch allocation limit.", buf.offer( 3 ) );
        assertTrue( buf.ensureCapacity( 1 ) );
        
        buf = new RingList<Integer>( 1, 200 );
        assertTrue( buf.ensureCapacity( 200 ) );
    }
    ...
    public void testSpeed() {
        ...
        for( int type = 0; type < 2; type++ ) {
            ...
            System.out.println( "RingList   : " + (dur[0] / 1000000000.0) + " seconds" );
            System.out.println( "ArrayList  : " + (dur[1] / 1000000000.0) + " seconds" );
            System.out.println( "LinkedList : " + (dur[2] / 1000000000.0) + " seconds" );
            System.out.println();
        }
    }
    
    
    private static void randomQueueOp( Random rand, Queue<Integer> a, int range ) {
    ...
    private static void randomListOp( Random rand, List<Integer> a, int range, boolean middleModifications ) {
        //int maxOp = middleModifications ? 9 : 7;
